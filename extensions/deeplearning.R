...
ks.deep_learning = function(selected_miRNAs = ".", wd = getwd(),
  ...
  final <- foreach(i=as.numeric(start):as.numeric(end), .combine=rbind, .verbose=F, .inorder=F, .export = ls()
    ...
    sink() 
    sink(type="message")
    message(paste0("miRNAselector: Finished training ",model_id, " : training_acc: ", tempwyniki[1, "training_Accuracy"], " | testing_acc:", tempwyniki[1, "test_Accuracy"], " ==> worth_saving: ", tempwyniki[1, "training_Accuracy"]>save_threshold_trainacc & tempwyniki[1, "test_Accuracy"]>save_threshold_testacc))
    #dev.off()
