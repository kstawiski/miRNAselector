...
rmarkdown::render("/miRNAselector/1_preprocessing.Rmd", output_file = "1_preprocessing.html", output_dir = "/miRNAselector");
file.copy("/task.log", "/miRNAselector/1_preprocessing.Rmd", overwrite = TRUE);
cat("[miRNAselector: TASK COMPLETED]"); writeLines("[2] PREPROCESSED", "var_status.txt", sep="");' 2>&1 | tee -a /task.log