...
Rscript --verbose -e 'library(tidyverse); library(remotes); update.packages(ask = F); library(BiocManager); install(ask = F); install_github("STATWORX/bounceR"); install_github("Thie1e/cutpointr"); install_github("vqv/ggbiplot"); install_github("kstawiski/miRNAselector")' 2>&1 | tee -a /update.log
