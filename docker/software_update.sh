...
Rscript --verbose -e 'library(remotes); library(devtools); update.packages(ask = F); library(BiocManager); install(ask = F); source_url("https://raw.githubusercontent.com/kstawiski/miRNAselector/master/vignettes/setup.R")' 2>&1 | tee -a /update.log
