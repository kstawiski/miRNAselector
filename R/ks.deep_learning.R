                            ...
                            keras_threads = ceiling(parallel::detectCores()/2), start = 1, end = nrow(hyperparameters), output_file = "deeplearning_results.csv", save_all_vars = F)
{
  ...
  #cl <- makePSOCKcluster(useXDR = TRUE, keras_threads) #not to overload your computer
  cl = makePSOCKcluster(useXDR = TRUE, keras_threads, outfile=paste0("temp/", ceiling(as.numeric(Sys.time())), "deeplearning_cluster.log"))
  suppressMessages(library(doParallel))
