...
#'
#' @param wd Working directory here `ks.miRNAselector` was also working.
#' @param input_fomulas List of formulas as created by `ks.miRNAselector` or `ks.merge_formulas`. Those formulas will be check in benchmark.
...
ks.benchmark = function(wd = getwd(), search_iters = 2000, keras_epochs = 5000, keras_threads = floor(parallel::detectCores()/2), search_iters_mxnet = 5000,
  ...
  for (ii in 1:length(algorytmy)) {
    ...
    if(grepl("Keras",algorytm)) {
      cl <- makePSOCKcluster(useXDR = FALSE, keras_threads)
      registerDoParallel(cl)
      # on.exit(stopCluster(cl))
    } else {
      cl <- makePSOCKcluster(useXDR = FALSE, cores-1)
      registerDoParallel(cl)
      # on.exit(stopCluster(cl))
    }
