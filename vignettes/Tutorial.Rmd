...
```{r}
library(miRNAselector)
selected_features = ks.miRNAselector(wd = getwd(), m = c(1:3,51:52), max_iterations = 1, stamp = "tutorial") # For the sake of this tutorial and vignette building we will use only few fastest methods. The m parameter defines what methods will be tested. See more details below.
```
...
```{r}
daneDFS = data.table::fread("daneDFS.csv")
```
