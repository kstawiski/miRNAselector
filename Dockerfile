...
RUN Rscript -e "library(keras); install_keras();" && Rscript -e "library(miRNAselector); ks.setup();" && pip install jupyter_contrib_nbextensions && jupyter contrib nbextension install && pip install nbresuse[resources]
