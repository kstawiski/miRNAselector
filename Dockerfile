...
RUN Rscript -e "library(keras); install_keras();" && Rscript -e "library(miRNAselector); ks.setup();" && conda install -c conda-forge jupyter_contrib_nbextensions nbresuse && jupyter contrib nbextension install
