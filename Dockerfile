...
RUN apt-get update --fix-missing && \
    apt-get install -y libffi-dev libssl-dev wget bzip2 ca-certificates build-essential cmake git unzip pkg-config libopenblas-dev liblapack-dev libhdf5-serial-dev python-h5py graphviz libglib2.0-0 libxext6 libsm6 libxrender1 git mercurial subversion gfortran-7 gcc-7 && apt-get clean && \
    pip install --upgrade pip tensorflow keras utils np_utils setuptools notebook jupytext
